...
float us;
static uint32_t u32nu_counter_5ms = 0;
...
void scheduler_init(void)
{
    ...
    htim2.Init.Period = 5000 - 1; // 5ms 주기
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
    	scheduler_update_flags();  // 5ms마다 호출됨
    }
}
...
void scheduler_update_flags(void)
{
    ...
    if (u32nu_counter_5ms % 3 == 0){
    	st_scheduling_info.u8nu_scheduling_15ms_flag = 1;
    	st_scheduling_info.u8nu_scheduling_5ms_flag = 1;
    }

    if (u32nu_counter_5ms % 4 == 0){
    	st_scheduling_info.u8nu_scheduling_20ms_flag = 1;
    	st_scheduling_info.u8nu_scheduling_5ms_flag = 0;
    	st_scheduling_info.u8nu_scheduling_15ms_flag = 0;
